...
const saveTimerState = (state) => {
  try {
    localStorage.setItem(TIMER_STORAGE_KEY, JSON.stringify({ ...state, savedAt: Date.now() }));
...
const loadTimerState = () => {
  ...
  } catch (e) { console.error('Failed to load timer state:', e); return null; }
};

const clearTimerState = () => {
  try { localStorage.removeItem(TIMER_STORAGE_KEY); } catch (e) { console.error('Failed to clear timer state:', e); }
};
...
const DragProvider = ({ children, onDrop }) => {
  ...
  return (
    <DragContext.Provider value={{ draggedBlock, setDraggedBlock, dropTarget, setDropTarget, onDrop }}>
      {children}
...
// ============================================
// SOUND HOOK (with repeating alarm)
// ============================================
...
const PomodoroTimer = ({ onComplete, currentTask, preferences }) => {
  ...
  useEffect(() => {
    ...
    if (savedState) {
      ...
      if (savedState.isRunning && savedState.endTime) {
        ...
        else { setTimeLeft(durations[savedState.mode] || durations.work); setIsRunning(false); }
      } else { setTimeLeft(savedState.timeLeft || durations[savedState.mode] || durations.work); setIsRunning(false); }
    }
  ...
  const acknowledgeAlarm = () => {
    stopAlarm();
    setIsAlarming(false);
    if (mode === 'work') { setMode('shortBreak'); setTimeLeft(durations.shortBreak); }
  ...
  const toggleTimer = () => {
    if (!isRunning) { playSound('start'); setEndTime(Date.now() + (timeLeft * 1000)); setIsRunning(true); }
    else { clearInterval(intervalRef.current); setTimeLeft(calculateRemainingTime(endTime)); setEndTime(null); setIsRunning(false); }
  };
  ...
  return (
    <div className="timer-container" style={{ background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)', borderRadius: '24px', padding: '32px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '24px', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      <div className="timer-mode-buttons" style={{ display: 'flex', gap: '8px', background: 'rgba(0,0,0,0.3)', padding: '6px', borderRadius: '12px' }}>
        {Object.keys(durations).map((m) => (
          <button key={m} onClick={() => switchMode(m)} style={{ padding: '10px 20px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontFamily: "'JetBrains Mono', monospace", fontSize: '12px', fontWeight: '600', transition: 'all 0.3s ease', background: mode === m ? modeColors[m] : 'transparent', color: mode === m ? '#fff' : 'rgba(255,255,255,0.5)' }}>{modeLabels[m]}</button>
        ))}
      </div>
      ...
      <div className="timer-circle" style={{ position: 'relative', width: '220px', height: '220px' }}>
        ...
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center' }}>
          ...
          {currentTask && <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.6)', fontFamily: "'Space Grotesk', sans-serif", marginTop: '4px', maxWidth: '140px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{currentTask}</div>}
        </div>
      </div>

      <div style={{ display: 'flex', gap: '16px' }}>
        {isAlarming ? (
          <button onClick={acknowledgeAlarm} style={{ padding: '14px 32px', border: 'none', borderRadius: '12px', cursor: 'pointer', fontFamily: "'Space Grotesk', sans-serif", fontSize: '16px', fontWeight: '700', letterSpacing: '2px', background: 'linear-gradient(135deg, #FF6B6B 0%, #ff4757 100%)', color: '#fff', boxShadow: '0 10px 30px rgba(255, 107, 107, 0.5)', animation: 'pulse 1s infinite' }}>🔔 STOP ALARM</button>
        ) : (
          <>
...
// Rest of components follow (TimeBlock, DroppableCell, EditBlockModal, TimerSettingsModal, AddBlockModal, AnalyticsDashboard, App)
// Due to size limits, see full file in repository
